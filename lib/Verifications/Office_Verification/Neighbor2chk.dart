...
class OfficeNeighborVerification2 extends StatefulWidget {
  const OfficeNeighborVerification2(
      {super.key,
  ...
  final int InqID;
  @override
  State<OfficeNeighborVerification2> createState() =>
      _OfficeNeighborVerification2State();
}

class _OfficeNeighborVerification2State
    extends State<OfficeNeighborVerification2> {
  ...
  MarketeCheck N2market = MarketeCheck();
  saveMarketCheckData() {
    try {
      MarketeCheck check = MarketeCheck(
          NeighborsTwoName: neighborNameController.text.toString(),
          NeighborsTwoAddress: neighborAddressController.text.toString(),
          NeighborsTwoKnowsApplicant: neighborKnowledgeValue,
          NeighborsTwoKnowsHowLong:
              knowledgeofNeighborController.text.toString(),
          NeighborsTwoBusinessEstablishedSinceMarketeCheck:
              establishedController.text.toString(),
          NeighborsTwoNeighborComments: commentController.text.toString());
      N2market = check;
    ...
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
            ...
            title: Text(
              "Neighbor/Market Check 2 (${widget.typeOfPerson})",
              style: TextStyle(
                  color: Colors.white,
                                  ...
                                  builder: (BuildContext context) {
                                    return SalariedDialog(
                                      ...
                                      await DBHelper.updateMarketchk2data(
                                          widget.officewrkid, N2market);
                                      // DBHelper.createOfficeMarketCheckdb(
                                      //     marketeCheck);
                        ...
                        onPressed: () {
                          try {
                            if (_OfficeNeighborformKey.currentState!
                                    .validate() &&
                                neighborKnowledgeValue.isNotEmpty) {
                              showDialog(
                                  context: context,
                                  builder: (BuildContext context) {
                                    return finalDialog(
